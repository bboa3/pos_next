/**
 * @fileoverview Translation system for POS MZ with offline support.
 *
...
const FALLBACK_LOCALE = "pt-MZ"
const SUPPORTED_TRANSLATION_LOCALES = ["pt-MZ"] as const
const CANONICAL_LOCALE_MAP: Record<string, string> = SUPPORTED_TRANSLATION_LOCALES.reduce(
  (acc, code) => {
...
const canonicalizeLocale = (locale?: string | null): string => {
  if (!locale) return FALLBACK_LOCALE
  const value = locale.toString().trim()
 ...
 * @example
 * // Simple translation
 * __('Save')  // → "Guardar" (Portuguese)
 *
 * // With placeholders
 * __('Hello {0}', { 0: 'Mundo' })  // → "Olá Mundo"
 *
 ...
 * @returns Canonical locale code
 */
const getLocale = (): string => {
  if (typeof window === "undefined") return FALLBACK_LOCALE

  const bootLang = (window as any)?.frappe?.boot?.lang
  const storedLang = window.localStorage?.getItem("pos_next_language")

  return canonicalizeLocale(bootLang || storedLang || FALLBACK_LOCALE)
}
 ...
 *
 * @param locale - Target locale code (e.g., "pt-MZ")
 * @returns Promise that resolves when translations are loaded
 *
 * @example
 * await changeLanguage('pt-MZ')
 */
export async function changeLanguage(locale: string): Promise<void> {
  const targetLocale = canonicalizeLocale(locale)
  const success = await loadLocale(targetLocale, { preferCache: true, forceNetwork: true })
  if (!success) fallbackFetch(targetLocale)
}
