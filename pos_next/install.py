"""
Installation and Migration hooks for POS MZ
"""
import frappe
import logging
...
def setup_default_print_format(quiet=False):
	"""
	Set POS MZ Receipt as default print format for POS Profiles if not already set
	...
	try:
		...
		if not frappe.db.exists("Print Format", "POS MZ Receipt"):
			if not quiet:
				log_message("POS MZ Receipt print format not found, skipping default setup", level="warning")
			return
		...
		if pos_profiles:
			...
			for profile in pos_profiles:
				try:
					...
					doc.print_format = "POS MZ Receipt"
					doc.flags.ignore_permissions = True
					doc.flags.ignore_mandatory = True
					doc.save()
					if not quiet:
