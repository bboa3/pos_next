"""
Uninstallation hooks for POS MZ
"""
import frappe
import logging
...
def before_uninstall():
	...
	"""
	try:
		log_message("Starting POS MZ uninstallation", level="info")
		...
		log_message("POS MZ uninstalled successfully", level="success")
		log_message("All custom fields and configurations have been removed", level="info")

	except Exception as e:
		frappe.db.rollback()
		frappe.log_error(
			title="POS MZ Uninstallation Error",
			message=frappe.get_traceback()
		)
		log_message(f"Error during POS MZ uninstallation: {str(e)}", level="error")
		raise


def remove_custom_fields():
	"""
	Remove all custom fields created by POS MZ
	"""
	try:
...
def remove_print_formats():
	"""
	Remove all print formats created by POS MZ
	"""
	try:
...
def reset_pos_profiles():
	"""
	Reset POS Profile configurations set by POS MZ
	"""
	try:
		log_message("Resetting POS Profile configurations", level="info")

		# Find POS Profiles using POS MZ print format
		pos_profiles = frappe.get_all(
			"POS Profile",
			filters={"print_format": "POS MZ Receipt"},
			fields=["name"]
		)

		if not pos_profiles:
			log_message("No POS Profiles using POS MZ configurations", level="info", indent=1)
			return
...
def get_custom_fields_for_cleanup():
	...
	Returns list of field names that belong to POS MZ
	"""
	custom_fields = []

	# Always safe to remove (POS MZ specific)
