# POS MZ - Version Control System

This document explains the version control mechanism implemented in POS MZ.

## Overview

POS MZ uses a comprehensive version control system that tracks both application versions and build versions for effective cache busting and version management.
...
- **Location**: POS MZ uses `public/pos/` for build assets
- **Fallback**: POS MZ checks manifest.webmanifest mtime as additional fallback
- **Script**: POS MZ includes automated version bump script
- **Documentation**: POS MZ has comprehensive version management guide
